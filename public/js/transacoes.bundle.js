;(function(window){
    ...
    async function renderTransacaoDetailsTo(containerId, t){
        if(!t) return;
    ...
    let moeda = (t.moeda && (t.moeda.codigo || t.moeda.nome)) || t.moeda_codigo || null;
        ...
        function findVisibleSummary(){
            ...
            for(const el of candidates){ if(el.offsetParent === null) continue; return el; }
            return null;
        }
        ...
        if(summaryEl){
            ...
            for(const dtEl of dts){ const key = (dtEl.textContent || '').trim().toLowerCase(); const dd = dtEl.nextElementSibling; if(!dd) continue; if(key.startsWith('conta')) acctFromSumm = dd.textContent.trim(); if(key.startsWith('titular')) titularFromSumm = dd.textContent.trim(); if(key.startsWith('moeda')) moedaFromSumm = dd.textContent.trim(); }
        }
    ...
    let html = '';
    html += '<div class="invoice p-3">';
    html += '<div class="mb-3"><h4>' + title + '</h4></div>';
    html += '<dl class="row">';
    html += '<dt class="col-sm-3">ID</dt><dd class="col-sm-9">' + id + '</dd>';
    html += '<dt class="col-sm-3">Data / Hora</dt><dd class="col-sm-9">' + dt + '</dd>';
    html += '<dt class="col-sm-3">Conta</dt><dd class="col-sm-9">' + accountNumber + '</dd>';
    html += '<dt class="col-sm-3">Titular</dt><dd class="col-sm-9">' + titular + '</dd>';
    html += '<dt class="col-sm-3">Valor</dt><dd class="col-sm-9">' + valor + '</dd>';
    html += '<dt class="col-sm-3">Moeda</dt><dd class="col-sm-9">' + moeda + '</dd>';
    if(t.conta && (typeof t.conta.saldo !== 'undefined')){ html += '<dt class="col-sm-3">Saldo disponível</dt><dd class="col-sm-9">' + Number(t.conta.saldo).toFixed(2) + '</dd>'; }
    html += '<dt class="col-sm-3">Descrição</dt><dd class="col-sm-9">' + descricao + '</dd>';
    html += '</dl>';
    html += '<div class="mt-3"><small class="text-muted">Este documento comprova que a operação foi registrada no sistema.</small></div>';
    html += '</div>';
