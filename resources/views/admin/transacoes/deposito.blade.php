...
<div class="card">
    <div class="card-body">
        <h5>Depósito</h5>
        <p class="text-muted">Formulário para efetuar depósitos em contas.</p>
...
document.addEventListener('DOMContentLoaded', function(){
    ...
    async function verifyAccount(numero, role){
        ...
        try{
            ...
            if(conta && conta.moeda){ const s = document.getElementById('deposit_moeda'); if(s){ s.innerHTML = ''; const opt = document.createElement('option'); opt.value = conta.moeda.id; opt.textContent = (conta.moeda.codigo?conta.moeda.codigo+' - ':'') + (conta.moeda.nome||''); s.appendChild(opt); s.value = conta.moeda.id; } }

            const input = document.querySelector('.conta-input[data-role="deposit"]'); if(input) input.dataset.contaId = conta.id;
            try{ document.getElementById('deposit_conta_id').value = conta.id; }catch(e){}
            const body = document.querySelector('#op_deposit .op_body'); if(body) body.style.display = 'block';
            return { conta };
        }catch(e){
            const infoEl = document.getElementById(role + '_account_info'); if(infoEl) infoEl.textContent = 'Conta não encontrada';
            const input = document.querySelector('.conta-input[data-role="deposit"]'); if(input) delete input.dataset.contaId;
            try{ document.getElementById('deposit_conta_id').value = ''; }catch(e){}
            const body = document.querySelector('#op_deposit .op_body'); if(body) body.style.display = 'none';
    ...
    try{
        ...
        const numero = params.get('numero_conta') || params.get('numero') || '';
        if(numero){ const input = document.querySelector('.conta-input[data-role="deposit"]'); if(input){ input.value = numero; verifyAccount(numero, 'deposit'); } }
    }catch(e){}
    ...
    document.getElementById('op_deposit').addEventListener('submit', async function(e){ e.preventDefault(); try{ const formData = new FormData(this); const data = Object.fromEntries(formData.entries()); const contaId = formData.get('conta_id') || (this.querySelector('.conta-input[data-role="deposit"]')?.dataset.contaId); if(!contaId) return setOpsAlert('Verifique a conta antes de submeter', 'danger'); const btn = this.querySelector('button[type="submit"], button.btn-success'); if(btn){ btn.disabled = true; var old = btn.innerHTML; btn.innerHTML='Processando...'; } const resp = await postJson('/api/contas/' + contaId + '/depositar', { valor: data.valor, moeda_id: data.moeda_id, descricao: data.descricao, referencia_externa: data.referencia_externa }); setOpsAlert(resp.message || 'Depósito efetuado','success'); if(resp.transacao && resp.transacao.id){ const r2 = await fetch('/api/transacoes/' + resp.transacao.id); if(r2.ok){ const json = await r2.json(); renderTransacaoDetails(json); } } if(btn){ btn.disabled = false; btn.innerHTML = old; } }catch(err){ setOpsAlert(err.message || 'Erro', 'danger'); } });
