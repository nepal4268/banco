...
document.addEventListener('DOMContentLoaded', function(){
    ...
    document.getElementById('op_transfer').addEventListener('submit', async function(e){ e.preventDefault(); try{ const formData = new FormData(this); const data = Object.fromEntries(formData.entries()); const origemInput = this.querySelector('.conta-input[data-role="transfer-origem"]'); const destinoInput = this.querySelector('.conta-input[data-role="transfer-destino"]'); const contaOrigemId = formData.get('conta_origem_id') || (origemInput?.dataset.contaId); const contaDestinoId = formData.get('conta_destino_id') || (destinoInput?.dataset.contaId); if(!contaOrigemId || !contaDestinoId) return setOpsAlert('Verifique origem e destino antes de submeter', 'danger'); const valorNum = Number(data.valor || 0); if(valorNum <= 0) return setOpsAlert('Valor deve ser maior que zero','danger'); const saldoOrig = Number(document.querySelector('#transfer_account_summary_body dd:nth-of-type(2)')?.textContent || 0); if(!isNaN(saldoOrig) && valorNum > saldoOrig) return setOpsAlert('Valor não pode ser maior que o saldo disponível na conta de origem','danger'); if(!confirm('Confirmar envio do PIX?')) return; const btn = this.querySelector('button[type="submit"]'); if(btn){ btn.disabled = true; var old = btn.innerHTML; btn.innerHTML='Processando...'; } await new Promise(r=> setTimeout(r, 2000)); const endpoint = '/api/transacoes/transferir'; const payload = { conta_origem_id: contaOrigemId, conta_destino_id: contaDestinoId, valor: data.valor, moeda_id: data.moeda_id, descricao: data.descricao, tipo: 'PIX' }; const resp = await window.Transacoes.postJson(endpoint, payload); setOpsAlert(resp.message || 'PIX efetuado','success'); if(resp.transacao && resp.transacao.id && window.Transacoes && window.Transacoes.renderTransacaoDetailsTo){ window.Transacoes.renderTransacaoDetailsTo('last_operation_details', resp.transacao); } this.reset(); const bodyEl = document.querySelector('#op_transfer .op_body'); if(bodyEl) bodyEl.style.display = 'none'; document.getElementById('transfer_account_summary').style.display = 'none'; document.getElementById('last_operation_card').style.display = 'block'; if(btn){ btn.disabled = false; btn.innerHTML = old; } }catch(err){ setOpsAlert(err.message || 'Erro', 'danger'); } });
